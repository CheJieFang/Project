...
class Slide{
	constructor(box, w, h, data, btnSize, now, ms, dire){
		...
		this.box.onmouseover = ()=>{
			...
			startMove(this.p1, {"opacity":40});
			startMove(this.p2, {"opacity":40});
		}
		this.box.onmouseout = ()=>{
			this.next();
	...
	createHtml(){
		...
		this.data.forEach(obj=>{
			...
			ul.appendChild(li);
			[li.style.width, li.style.height] = [this.w/4+"px", this.h+"px"];
			...
			a.appendChild(img);
			[img.style.width, img.style.height] = [this.w/4+"px", this.h+"px"];
			img.src = obj.img;
		});
		// 无缝滚动，拷贝第一个ul>前四个li
		...
		ul.appendChild(li2);
		var li3 = ul.children[2].cloneNode(true);
		ul.appendChild(li3);
		var li4 = ul.children[3].cloneNode(true);
		ul.appendChild(li4);
		// ol 轮播图按钮
	...
	// 当点击按钮时，切换图片
	tab(){
		if( this.now == this.len/4 ){
			this.now = 0;
			startMove(this.ul, {"left":this.len/4*-this.w}, ()=>{
